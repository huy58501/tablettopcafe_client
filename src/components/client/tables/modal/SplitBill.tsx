...
import { FaMoneyBill, FaCheck, FaTimes, FaArrowRight, FaArrowLeft } from 'react-icons/fa';
...
const SplitBill: React.FC<SplitBillProps> = ({ isOpen, onClose, order, onConfirm }) => {
  ...
  useEffect(() => {
    if (order) {
      const initialItems = order.orderItems.map(item => ({
        ...
        splitQuantity: item.quantity // Set initial splitQuantity to full quantity for Bill 1
      }));
  ...
  const moveItemToBill2 = (item: SplitItem) => {
    if (item.splitQuantity > 0) { // Changed condition to check if there are items to move
      // Update quantities
      const updatedBill1Items = bill1Items.map(i => {
        if (i.id === item.id) {
  ...
  const moveItemToBill1 = (item: SplitItem) => {
    if (item.splitQuantity > 0) {
      // Update quantities
      const updatedBill2Items = bill2Items.map(i => {
        if (i.id === item.id) {
          const newSplitQuantity = i.splitQuantity - 1;
          return { ...i, splitQuantity: newSplitQuantity };
        }
  ...
  const BillItem = ({ item, onMove, direction }: { item: SplitItem; onMove: () => void; direction: 'left' | 'right' }) => (
    <motion.div
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-gray-700">Bill 1</h3>
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium text-gray-700">Bill 2</h3>
                <span className="text-lg font-semibold text-blue-600">
